...
mkdir -p results
cd results
resultPath=hive_mr_normal_query_sf"$1"_result.txt
rm -f $resultPath
touch $resultPath
...
echo $(date '+%d/%m/%Y %H:%M:%S.%3N')|& tee -a $resultPath
/opt/hive/bin/beeline -u jdbc:hive2://localhost:10000 -e "set hive.execution.engine=mr; select s_acctbal,s_name, n_name,p_partkey,p_mfgr,s_address,s_phone,s_commentfrom p_part inner join ps_partsuppon p_partkey = ps_partkey and p_size = 15 and p_type like '%BRASS'inner join s_supplieron s_suppkey = ps_suppkeyinner join n_nationon s_nationkey = n_nationkeyinner join r_regionon n_regionkey = r_regionkey and r_name = 'EUROPE'whereps_supplycost = (select min(ps_supplycost)from ps_partsupp, s_supplier, n_nation, r_regionwherep_partkey = ps_partkey and s_suppkey = ps_suppkeyand s_nationkey = n_nationkeyand n_regionkey = r_regionkeyand r_name = 'EUROPE') order by s_acctbal desc,n_name,s_name,p_partkey limit 100;"|& tee -a $resultPath
echo $(date '+%d/%m/%Y %H:%M:%S.%3N')|& tee -a $resultPath
