...
if [ ! -d $1 ] && [ ! -d $2 ]; then
cd /data/results
resultPath=hive_mr_normal_query_sf"$1"_"$2"_result_explain.txt
rm -f $resultPath
touch $resultPath
...
echo $(date '+%d/%m/%Y %H:%M:%S.%3N')|& tee -a $resultPath
/opt/hive/bin/beeline -u jdbc:hive2://localhost:10000 -e " set hive.execution.engine=$2; explain select o_orderpriority, count(*) as order_count from o_orders where o_orderdate >= date '1993-07-01' and o_orderdate < date '1993-07-01' + interval '3' month and exists (select * from l_lineitem where l_orderkey = o_orderkey and l_commitdate < l_receiptdate) group by o_orderpriority order by o_orderpriority;"|& tee -a $resultPath
echo $(date '+%d/%m/%Y %H:%M:%S.%3N')|& tee -a $resultPath
...
echo $(date '+%d/%m/%Y %H:%M:%S.%3N')|& tee -a $resultPath
/opt/hive/bin/beeline -u jdbc:hive2://localhost:10000 -e " set hive.execution.engine=$2; set hive.auto.convert.join=false; explain select n_name,sum(l_extendedprice * (1 - l_discount)) as revenue from c_customer, o_orders, l_lineitem, s_supplier, n_nation, r_region where c_custkey = o_custkey and l_orderkey = o_orderkey and l_suppkey = s_suppkey and c_nationkey = s_nationkey and s_nationkey = n_nationkey and n_regionkey = r_regionkey and r_name = 'ASIA' and o_orderdate >= date '1994-01-01' and o_orderdate < date '1994-01-01' + interval '1' year group by n_name order by revenue desc; set hive.auto.convert.join=true;"|& tee -a $resultPath
echo $(date '+%d/%m/%Y %H:%M:%S.%3N')|& tee -a $resultPath
...
echo $(date '+%d/%m/%Y %H:%M:%S.%3N')|& tee -a $resultPath
/opt/hive/bin/beeline -u jdbc:hive2://localhost:10000 -e " set hive.execution.engine=$2; explain select sum(l_extendedprice * l_discount) as revenue from l_lineitem where l_shipdate >= date '1994-01-01' and l_shipdate < date '1994-01-01' + interval '1' year and l_discount between 0.06 - 0.01 and 0.06 + 0.01 and l_quantity < 24;"|& tee -a $resultPath
echo $(date '+%d/%m/%Y %H:%M:%S.%3N')|& tee -a $resultPath
...
echo $(date '+%d/%m/%Y %H:%M:%S.%3N')|& tee -a $resultPath
/opt/hive/bin/beeline -u jdbc:hive2://localhost:10000 -e " set hive.execution.engine=$2; set hive.auto.convert.join=false; explain select c_custkey, c_name, sum(l_extendedprice * (1 - l_discount)) as revenue,c_acctbal,n_name,c_address,c_phone,c_comment from c_customer, o_orders, l_lineitem, n_nation where c_custkey = o_custkey and l_orderkey = o_orderkey and o_orderdate >= date '1993-10-01' and o_orderdate < date '1993-10-01' + interval '3' month and l_returnflag = 'R' and c_nationkey = n_nationkey group by c_custkey, c_name, c_acctbal,c_phone,n_name,c_address, c_comment order by revenue desc limit 20; set hive.auto.convert.join=true;"|& tee -a $resultPath
echo $(date '+%d/%m/%Y %H:%M:%S.%3N')|& tee -a $resultPath
...
echo $(date '+%d/%m/%Y %H:%M:%S.%3N')|& tee -a $resultPath
/opt/hive/bin/beeline -u jdbc:hive2://localhost:10000 -e " set hive.execution.engine=$2; set hive.auto.convert.join=false; explain select l_shipmode, sum(case when o_orderpriority = '1-URGENT' or o_orderpriority = '2-HIGH' then 1 else 0 end) as high_line_count,sum(case when o_orderpriority <> '1-URGENT' and o_orderpriority <> '2-HIGH' then 1 else 0 end) as low_line_count from o_orders, l_lineitem where o_orderkey = l_orderkey and l_shipmode in ('MAIL', 'SHIP') and l_commitdate < l_receiptdate and l_shipdate < l_commitdate and l_receiptdate >= date '1994-01-01' and l_receiptdate < date '1994-01-01' + interval '1' year group by l_shipmode order by l_shipmode; set hive.auto.convert.join=true;"|& tee -a $resultPath
echo $(date '+%d/%m/%Y %H:%M:%S.%3N')|& tee -a $resultPath
...
echo $(date '+%d/%m/%Y %H:%M:%S.%3N')|& tee -a $resultPath
/opt/hive/bin/beeline -u jdbc:hive2://localhost:10000 -e " set hive.execution.engine=$2; explain select c_count, count(*) as custdist from (select c_custkey,count(o_orderkey) as c_count from c_customer left outer join o_orders on c_custkey = o_custkey and o_comment not like '%special%requests%'group by c_custkey) as c_orders group by c_count order by custdist desc, c_count desc;"|& tee -a $resultPath
echo $(date '+%d/%m/%Y %H:%M:%S.%3N')|& tee -a $resultPath
...
echo $(date '+%d/%m/%Y %H:%M:%S.%3N')|& tee -a $resultPath
/opt/hive/bin/beeline -u jdbc:hive2://localhost:10000 -e " set hive.execution.engine=$2; explain select 100.00 * sum(case when p_type like 'PROMO%' then l_extendedprice * (1 - l_discount) else 0 end) / sum(l_extendedprice * (1 - l_discount)) as promo_revenue from l_lineitem, p_part where l_partkey = p_partkey and l_shipdate >= date '1995-09-01' and l_shipdate < date '1995-09-01' + interval '1' month;"|& tee -a $resultPath
echo $(date '+%d/%m/%Y %H:%M:%S.%3N')|& tee -a $resultPath
...
echo $(date '+%d/%m/%Y %H:%M:%S.%3N')|& tee -a $resultPath
/opt/hive/bin/beeline -u jdbc:hive2://localhost:10000 -e " set hive.execution.engine=$2; explain select sum(l_extendedprice) / 7.0 as avg_yearly from l_lineitem, p_part where p_partkey = l_partkey and p_brand = 'Brand#23' and p_container = 'MED BOX' and l_quantity < (select 0.2 * avg(l_quantity) from l_lineitem where l_partkey = p_partkey);"|& tee -a $resultPath
echo $(date '+%d/%m/%Y %H:%M:%S.%3N')|& tee -a $resultPath
...
Paramter 2 for execution engine."
echo "Example ./hive_mr_normal_query.sh 1 tez"
exit 2
fi